...
//   double*  pagerank_old=allocateMemoryDouble(nodes);   
   ...
   cudaMalloc(&in_d, nodes * sizeof(int));
   cudaMemcpy(in_d, indegree_count, nodes * sizeof(int), cudaMemcpyHostToDevice);
   cudaMalloc(&out_d, nodes * sizeof(int));
   cudaMemcpy(out_d, outdegree_count, nodes * sizeof(int), cudaMemcpyHostToDevice);
   cudaMalloc(&run_d, nodes * sizeof(int));
   cudaMemcpy(run_d, running_edge_indices, nodes * sizeof(int), cudaMemcpyHostToDevice);
   cudaMalloc(&edges_d, edges * sizeof(int));
   cudaMemcpy(edges_d, edges_1D, edges * sizeof(int), cudaMemcpyHostToDevice);
   cudaMalloc(&pagerank_old_d,nodes * sizeof(double));
