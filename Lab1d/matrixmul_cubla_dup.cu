...
_data_type *readFile(char *fileName, int *rows, int *cols) {
   ...
   _data_type *mat2 = allocateMemory(rCount, cCount);
   for(int i = 0; i<rCount*cCount; i++) {
        mat2[rCount*(i%cCount) + i/cCount] = mat[i];
    }
...
void matrixMulOnDevice (_data_type *m, _data_type *n, _data_type *p, int rowsM, int colsM, int rowsN, int colsN) {
   ...
   cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, rowsM, colsN, colsM, &alpha, Md, rowsM, Nd, rowsN, &beta, Pd, rowsM);
